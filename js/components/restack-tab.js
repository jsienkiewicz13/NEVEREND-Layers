...
<button v-if="!restack.allPermUpgradesBought() && !isMeta" @click="restack.respecPermUpgrades()">Respec: get your spent <img alt="LC" class="inline" src="images/layercoin.png"/> back, and do a Re-End without reward</button>
...
<button class="restack" @click="restack.restack()" :disabled="!canRestack" v-if="!isMeta">Re-End<br/>+{{formatNumber(restack.getRestackGain(), 2, 0)}} <img alt="LC" class="inline" src="images/layercoin.png"/></button>
<p v-if="showMetaHint && !restack.canMeta()">Come back after reaching <resource-name :layerid="23"></resource-name></p>
<button v-if="restack.canMeta() && !isMeta" @click="restack.goMeta()" class="meta">Prestige is worthless now, we are neverending!!!</button>
</div>`
});
